var infoHref = './pinfo.html?nav=';
var dataname = 'produceData';
var produceData = [
    {
        title: '交通产品',
        subtext: 'traffic',
        children: [
            {
                title: '公交车载主机',
                img: './assets/img/produce/list/1.png',
                image: ['./assets/img/produce/bus1.jpg'],
                type: 1,
                summary: '内置海思高性能图像处理芯片，H.264编码，高压缩比，图像清晰支持8路AHD百万高清/模拟标清视音频输入，1路视音频同步输出,1路VGA输出，支持8路1080N高清实时本地录像',
                feature: [{
                    title: '产品功能特点',
                    content: [
                        {
                            title: '配置概述',
                            list: [
                                '内置海思高性能图像处理芯片，H.264编码，高压缩比，图像清晰',
                                '支持8路AHD百万高清/模拟标清视音频输入，1路视音频同步输出,1路VGA输出',
                                '支持8路1080N高清实时本地录像',
                                '支持8路720P高清实时本地录像',
                                '支持8路960H高清实时本地录像',
                            ]
                        },
                        {
                            title: '电源',
                            list: [
                                '专业车载电源设8—32V直流宽电压输入设计',
                                '欠压、短路、反接等多种保护电路，适用于各种车型',
                                '支持智能电源管理识别，低电自动关机，熄火低功耗',
                            ]
                        },
                        {
                            title: '数据存储',
                            list: [
                                '采用特殊文件管理机制，对数据进行加密，有效保护数据安全',
                                '专有磁盘坏道检测技术，既保证录像连续性，又延长磁盘寿命',
                                '内置超级电容，避免异常断电数据丢失和磁盘损坏',
                                '支持2.5寸硬盘，最大支持2TB',
                                '支持SD存储，最大支持256G',
                                '支持硬盘自动加热',
                            ]
                        },
                        {
                            title: '接口传输',
                            list: [
                                '支持GPS/BD定位，灵敏度高，定位快速',
                                '支持WIFI无线下载，支持802.11b/g/n，频率2.4GHz',
                                '内置3G/4G模块，支持LTE/HSUPA/HSDPA/WCDMA/EVDO',
                            ]
                        }
                    ]
                }],
                parameter: [{
                    title: '产品参数',
                    caption: {
                        name: '产品技术参数',
                        value: 'YZGJ-A1480'
                    },
                    thead: [
                        {
                            value: '项目',
                        },
                        {
                            value: '设备参数',
                        },
                        {
                            value: '性能指标',
                        },
                    ],
                    tbody: [
                        {
                            value: '系统',
                            rowspan: 5,
                            children: [
                                {
                                    value: '主处理器',
                                    children: [
                                        {
                                            value: 'Hi3520DV300',
                                        },
                                    ],
                                },
                                {
                                    value: '操作系统',
                                    children: [
                                        {
                                            value: '嵌入式Linux操作系统',
                                        },
                                    ],
                                },
                                {
                                    value: '操作语言',
                                    children: [
                                        {
                                            value: '中文/英文/繁体',
                                        },
                                    ],
                                },
                                {
                                    value: '操作界面',
                                    children: [
                                        {
                                            value: '图形化菜单操作界面，支持鼠标操作',
                                        },
                                    ],
                                },
                                {
                                    value: '密码安全',
                                    children: [
                                        {
                                            value: '用户密码、管理员密码两级管理',
                                        },
                                    ],
                                },
                            ]
                        },
                        {
                            value: '视音频',
                            rowspan: 10,
                            children: [
                                {
                                    value: '视频制式',
                                    children: [
                                        {
                                            value: 'PAL/NTSC',
                                        },
                                    ],
                                },
                                {
                                    value: '压缩标准',
                                    children: [
                                        {
                                            value: 'H.264',
                                        },
                                    ],
                                },
                                {
                                    value: '图像分辨率',
                                    children: [
                                        {
                                            value: '1080N/720P /960H/D1/CIF',
                                        },
                                    ],
                                },
                                {
                                    value: '回放质量',
                                    children: [
                                        {
                                            value: '1080N/720P /960H/D1/CIF',
                                        },
                                    ],
                                },
                                {
                                    value: '组合方式',
                                    children: [
                                        {
                                            value: '多种方式',
                                        },
                                    ],
                                },
                                {
                                    value: '解码能力',
                                    children: [
                                        {
                                            value: '1路1080n实时',
                                        },
                                    ],
                                },
                                {
                                    value: '录像质量',
                                    children: [
                                        {
                                            value: '1到6级可配',
                                        },
                                    ],
                                },
                                {
                                    value: '画面显示',
                                    children: [
                                        {
                                            value: '支持1、2、3、4、9画面显示',
                                        },
                                    ],
                                },
                                {
                                    value: '音频压缩',
                                    children: [
                                        {
                                            value: 'G.726',
                                        },
                                    ],
                                },
                                {
                                    value: '录音方式',
                                    children: [
                                        {
                                            value: '声音与视频同步录制',
                                        },
                                    ],
                                },
                            ]
                        },
                        {
                            value: '录像和回放',
                            rowspan: 6,
                            children: [
                                {
                                    value: '录像方式',
                                    children: [
                                        {
                                            value: '手动、报警',
                                        },
                                    ],
                                },
                                {
                                    value: '视频码率',
                                    children: [
                                        {
                                            value: '满帧4096Mbps，6级画质可选',
                                        },
                                    ],
                                },
                                {
                                    value: '音频码率',
                                    children: [
                                        {
                                            value: '8KB/s',
                                        },
                                    ],
                                },
                                {
                                    value: '存储介质',
                                    children: [
                                        {
                                            value: '硬盘+SD卡',
                                        },
                                    ],
                                },
                                {
                                    value: '录像查询',
                                    children: [
                                        {
                                            value: '可按通道、录像类型、报警类型进行检索',
                                        },
                                    ],
                                },
                                {
                                    value: '本地回放',
                                    children: [
                                        {
                                            value: '单路、按时间',
                                        },
                                    ],
                                },
                            ]
                        },
                        {
                            value: '软件升级',
                            children: [
                                {
                                    value: '升级方法',
                                    children: [
                                        {
                                            value: 'USB接口、无线网络、SD卡、硬盘',
                                        },
                                    ],
                                },
                            ]
                        },
                        {
                            value: '接口',
                            rowspan: 11,
                            children: [
                                {
                                    value: '音视频输入',
                                    children: [
                                        {
                                            value: '8路航空头接口',
                                        },
                                    ],
                                },
                                {
                                    value: '音视频输出',
                                    children: [
                                        {
                                            value: '1路VGA视频输出，1路航空头音视频输出',
                                        },
                                    ],
                                },
                                {
                                    value: '报警输入',
                                    children: [
                                        {
                                            value: '4个数字输入（四路可配置正/负触发）',
                                        },
                                    ],
                                },
                                {
                                    value: '磁盘接口',
                                    children: [
                                        {
                                            value: '2.5寸硬盘，最大支持2TB',
                                        },
                                    ],
                                },
                                {
                                    value: 'SD卡',
                                    children: [
                                        {
                                            value: '1个SDXC高速卡(最大支持256G)',
                                        },
                                    ],
                                },
                                {
                                    value: 'USB接口',
                                    children: [
                                        {
                                            value: '1个USB 2.0（支持全鼠标操作，USB延时自检） ',
                                        },
                                    ],
                                },
                                {
                                    value: '点火输入',
                                    children: [
                                        {
                                            value: '1路ACC信号',
                                        },
                                    ],
                                },
                                {
                                    value: '串口',
                                    children: [
                                        {
                                            value: '1路LVTTL电平',
                                        },
                                    ],
                                },
                                {
                                    value: 'LED指示灯',
                                    children: [
                                        {
                                            value: 'PWR/RUN',
                                        },
                                    ],
                                },
                                {
                                    value: '磁盘锁',
                                    children: [
                                        {
                                            value: '1个',
                                        },
                                    ],
                                },
                                {
                                    value: '调试串口',
                                    children: [
                                        {
                                            value: '1个',
                                        },
                                    ],
                                },
                            ]
                        },
                        {
                            value: '扩展功能',
                            rowspan: 3,
                            children: [
                                {
                                    value: 'GPS',
                                    children: [
                                        {
                                            value: '支持天线插入/拔出/短路检测',
                                        },
                                    ],
                                },
                                {
                                    value: '2G/3G/4G',
                                    children: [
                                        {
                                            value: '支持CDMA/EVDO/GPRS/WCDMA/FDD LTE/TDD LTE',
                                        },
                                    ],
                                },
                                {
                                    value: 'WIFI',
                                    children: [
                                        {
                                            value: '802.11b/g/n，频率2.4GHz',
                                        },
                                    ],
                                },
                            ]
                        },
                        {
                            value: '其它',
                            rowspan: 6,
                            children: [
                                {
                                    value: '电源输入',
                                    children: [
                                        {
                                            value: 'DC：8V~32V',
                                        },
                                    ],
                                },
                                {
                                    value: '电源输出',
                                    children: [
                                        {
                                            value: '5V 300mA',
                                        },
                                    ],
                                },
                                {
                                    value: '功耗',
                                    children: [
                                        {
                                            value: '待机3mA,最大功耗30W  @12V 2.5A @24V 1.25A',
                                        },
                                    ],
                                },
                                {
                                    value: '工作温度',
                                    children: [
                                        {
                                            value: '-20 --- 70℃',
                                        },
                                    ],
                                },
                                {
                                    value: '存储容量',
                                    children: [
                                        {
                                            value: '1080N   1.2G/小时/通道,720P    1G/小时/通道,960H    750M/小时/通道',
                                        },
                                    ],
                                },
                                {
                                    value: '尺寸(长x宽x高）',
                                    children: [
                                        {
                                            value: '162mm*180mm*50.5mm',
                                        },
                                    ],
                                },
                            ]
                        },


                    ]

                }]
            },
            {
                title: '公交车载司机操作屏',
                img: './assets/img/produce/list/2.png',
                image: ['./assets/img/produce/bus2.1.jpg',
                    './assets/img/produce/bus2.2.jpg'],
                type: 1,
                summary: '内置全志 V40，四核 ARM Cortex A7 1.5 GHz,内存：DDR3 1GB，内置存储：EMMC 8G，屏幕类型：液晶模组,  a-Si TFT-LCD',
                feature: [{
                    title: '产品功能特点',
                    content: [
                        {
                            title: '配置概述',
                            list: [
                                '内置全志 V40，四核 ARM Cortex A7 1.5 GHz',
                                '内存：DDR3 1GB',
                                '内置存储：EMMC 8G',
                                '屏幕类型：液晶模组,  a-Si TFT-LCD',
                                '分辨率：1280(RGB)X800 [WXGA]',
                                'IMEI:设备上装有模拟唯一通信编码（IMEI），方便设备正常运行管理',
                            ]
                        },
                        {
                            title: '电源',
                            list: [
                                '输入电压范围：DC6V~36V',
                                '规格输入电压：DC12V~24V',
                                '输入过压保护：>36V',
                                '输出电压：12V',
                                '开机延时功能：可延时 15 秒开机！',
                                '开机ACC 功能：接入车钥匙开关引线,当钥匙在①OFF 转到②ACC 时,电源板才有输出电压。',
                            ]
                        },
                    ]
                }],
                parameter: [{
                    title: '产品参数',
                    caption: {
                        name: '产品技术参数',
                        value: 'YZGJ-A1480'
                    },
                    thead: [
                        {
                            value: '项目',
                        },
                        {
                            value: '设备参数',
                        },
                        {
                            value: '性能指标',
                        },
                    ],
                    tbody: [
                        {
                            value: '系统',
                            rowspan: 4,
                            children: [
                                {
                                    value: '主处理器',
                                    children: [
                                        {
                                            value: '全志 V40，四核 ARM Cortex A7 1.5 GHz',
                                        },
                                    ],
                                },
                                {
                                    value: '操作系统',
                                    children: [
                                        {
                                            value: 'Google Android 6.0',
                                        },
                                    ],
                                },
                                {
                                    value: '操作语言',
                                    children: [
                                        {
                                            value: '中文/英文',
                                        },
                                    ],
                                },
                                {
                                    value: '操作界面',
                                    children: [
                                        {
                                            value: '图形化菜单操作界面，触控操作',
                                        },
                                    ],
                                },
                            ]
                        },
                        {
                            value: '视音频',
                            rowspan: 9,
                            children: [
                                {
                                    value: '屏幕类型',
                                    children: [
                                        {
                                            value: '液晶模组,   a-Si TFT-LCD',
                                        },
                                    ],
                                },
                                {
                                    value: '分辨率',
                                    children: [
                                        {
                                            value: '1280(RGB)×800   [WXGA]',
                                        },
                                    ],
                                },
                                {
                                    value: '视频播放',
                                    children: [
                                        {
                                            value: '支持wmv、avi、flv、rm、rmvb、mpeg 、ts、mp4等',
                                        },
                                    ],
                                },
                                {
                                    value: '网络支持',
                                    children: [
                                        {
                                            value: '以太网、支持WiFi',
                                        },
                                    ],
                                },
                                {
                                    value: '图片格式',
                                    children: [
                                        {
                                            value: '支持BMP、JPEG、PNG等',
                                        },
                                    ],
                                },
                                {
                                    value: '亮度',
                                    children: [
                                        {
                                            value: '500 (Typ.)',
                                        },
                                    ],
                                },
                                {
                                    value: '最佳角度',
                                    children: [
                                        {
                                            value: '全视角',
                                        },
                                    ],
                                },
                                {
                                    value: '可视角度',
                                    children: [
                                        {
                                            value: '85/85/85/85 (Typ.)(CR≥10)',
                                        },
                                    ],
                                },
                                {
                                    value: '表面硬度',
                                    children: [
                                        {
                                            value: '6H以上',
                                        },
                                    ],
                                },
                            ]
                        },
                        {
                            value: '接口',
                            rowspan: 8,
                            children: [
                                {
                                    value: 'MIC输入',
                                    children: [
                                        {
                                            value: 'MIC 1个',
                                        },
                                    ],
                                },
                                {
                                    value: '视频输入',
                                    children: [
                                        {
                                            value: 'TV输入1个，支持1280(RGB)×800',
                                        },
                                    ],
                                },
                                {
                                    value: 'USB接口',
                                    children: [
                                        {
                                            value: '2个USB2.0',
                                        },
                                    ],
                                },
                                {
                                    value: 'SD卡',
                                    children: [
                                        {
                                            value: 'TF卡',
                                        },
                                    ],
                                },
                                {
                                    value: '网络接口',
                                    children: [
                                        {
                                            value: '1个，10M/100M自适应以太网 ',
                                        },
                                    ],
                                },
                                {
                                    value: 'HDMI',
                                    children: [
                                        {
                                            value: '1个,支持分辨率1280(RGB)×800   [WXGA]',
                                        },
                                    ],
                                },
                                {
                                    value: 'RS232串口',
                                    children: [
                                        {
                                            value: 'RS232串口2个',
                                        },
                                    ],
                                },
                                {
                                    value: 'WIFI',
                                    children: [
                                        {
                                            value: '802.11b/g/n，频率2.4GHz',
                                        },
                                    ],
                                },
                            ]
                        },
                        {
                            value: '其它',
                            rowspan: 7,
                            children: [
                                {
                                    value: '电源输入',
                                    children: [
                                        {
                                            value: 'DC：DC6V~36V',
                                        },
                                    ],
                                },
                                {
                                    value: '规格输入电压',
                                    children: [
                                        {
                                            value: 'DC12V~24V',
                                        },
                                    ],
                                },
                                {
                                    value: '输入过压保护',
                                    children: [
                                        {
                                            value: '>36V',
                                        },
                                    ],
                                },
                                {
                                    value: '输出电压',
                                    children: [
                                        {
                                            value: '12V',
                                        },
                                    ],
                                },
                                {
                                    value: '工作温度',
                                    children: [
                                        {
                                            value: '-20℃~~70℃，≤ 85%RH',
                                        },
                                    ],
                                },
                                {
                                    value: '功耗',
                                    children: [
                                        {
                                            value: '24W',
                                        },
                                    ],
                                },
                                {
                                    value: '尺寸（长X宽X高）',
                                    children: [
                                        {
                                            value: '260*177*47mm',
                                        },
                                    ],
                                },
                            ]
                        },
                        {
                            value: '外壳',
                            rowspan: 5,
                            children: [
                                {
                                    value: '材质',
                                    children: [
                                        {
                                            value: '金属',
                                        },
                                    ],
                                },
                                {
                                    value: '固定方式',
                                    children: [
                                        {
                                            value: '管夹壁挂',
                                        },
                                    ],
                                },
                                {
                                    value: '本机尺寸',
                                    children: [
                                        {
                                            value: '260*177*47mm',
                                        },
                                    ],
                                },
                                {
                                    value: '遮阳罩尺寸',
                                    children: [
                                        {
                                            value: '263*51*81mm',
                                        },
                                    ],
                                },
                                {
                                    value: '管夹内径',
                                    children: [
                                        {
                                            value: '33mm',
                                        },
                                    ],
                                },
                            ]
                        },

                    ]

                }]

            },
            {
                title: '公交车载LED屏',
                img: './assets/img/produce/list/3.png',
                image: [
                    './assets/img/produce/bus3.1.jpg',
                    './assets/img/produce/bus3.2.jpg',
                    './assets/img/produce/bus3.3.jpg',
                    './assets/img/produce/bus3.4.jpg'
                ],
                type: 1,
                summary: '通过RS485接口连接车载LED屏通过智能车载一体机控制车载LED屏显示内容，车载LED屏可实时显示线路信息、始发站、终点站信息、实时到站信息和车辆转向、刹车等信息，并支持文明用语、广告模式轮播模式',
                feature: [{
                    title: '产品功能特点',
                    content: [
                        {
                            title: '',
                            list: [
                                '通过RS485接口连接车载LED屏通过智能车载一体机控制车载LED屏显示内容，车载LED屏可实时显示线路信息、始发站、终点站信息、实时到站信息和车辆转向、刹车等信息，并支持文明用语、广告模式轮播模式',
                                '通过司机操作屏司机可切换线路、广告模式、包车、退出运营等，车载LED屏同步接收智能车载一体机控制命令并将车载LED屏显示内容根据司机操作实时更改，可通过调度中心向智能车载一体机下发指令实时调整LED信息播放内容和工作模式。',
                            ]
                        },
                    ]
                }],
            },
            {
                title: '客流统计设备',
                img: './assets/img/produce/list/4.png',
                image: ['./assets/img/produce/bus4.1.jpg'],
                type: 1,
                summary: '采用领先的双目立体视觉技术，基于双镜头的立体摄像，获取目标的高度信息，结合智能跟踪算法分析行人的行为轨迹，从而精确计算出客流人数及行走方向',
                feature: [{
                    title: '产品功能特点',
                    content: [
                        {
                            title: '',
                            list: [
                                '支持分类统计人员进入、离开情况',
                                '支持实时数据上传和周期上传',
                                '支持高度过滤',
                                '支持开关门状态显示',
                                '支持小孩计数',
                                '支持反向计数',
                            ]
                        },


                    ]
                }],
                parameter: [{
                    title: '产品参数',
                    thead: [

                        {
                            value: '设备参数',
                        },
                        {
                            value: '性能指标',
                        },
                    ],
                    tbody: [

                        {
                            value: '名称',
                            children: [
                                {
                                    value: '参数',
                                },
                            ],
                        },
                        {
                            value: '存储',
                            children: [
                                {
                                    value: '支持内置存储（EMMC）',
                                },
                            ],
                        },
                        {
                            value: '最低照度',
                            children: [
                                {
                                    value: '彩色:0.002 Lux @ (F1.2, AGC ON)，0.007 Lux @ (F2.25, AGC ON) ; 黑白:0.0002 Lux @ (F1.2, AGC ON), xxx Lux @ (Fx, AGC ON)，0 Lux with IR',
                                },
                            ],
                        },
                        {
                            value: '镜头',
                            children: [
                                {
                                    value: '2.0mm@F2.25：水平视场角：108°，垂直视场角：84°，对角视场角：117°',
                                },
                            ],
                        },
                        {
                            value: '三轴或两轴调节角度',
                            children: [
                                {
                                    value: '水平调节：0°，上下调节：0-25°',
                                },
                            ],
                        },
                        {
                            value: '视频压缩标准',
                            children: [
                                {
                                    value: 'H.265/H.264 / MJPEG',
                                },
                            ],
                        },
                        {
                            value: '最大图像尺寸',
                            children: [
                                {
                                    value: '1920 x 1200',
                                },
                            ],
                        },
                        {
                            value: '存储功能',
                            children: [
                                {
                                    value: '支持Micro SD(即TF卡)/Micro SDHC /Micro SDXC卡(128GB或者256GB)断网本地存储及断网续传,NAS(NFS,SMB/CIFS均支持),配合海康黑卡支持SD卡加密及SD状态检测功能',
                                },
                            ],
                        },
                        {
                            value: '镜头',
                            children: [
                                {
                                    value: '2.0mm@F2.25：水平视场角：108°，垂直视场角：84°，对角视场角：117°',
                                },
                            ],
                        },
                        {
                            value: '通讯接口',
                            children: [
                                {
                                    value: '1个RJ45 10M / 100M自适应以太网口',
                                },
                            ],
                        },
                        {
                            value: '镜头',
                            children: [
                                {
                                    value: '2.0mm@F2.25：水平视场角：108°，垂直视场角：84°，对角视场角：117°',
                                },
                            ],
                        },
                        {
                            value: '音频接口:',
                            children: [
                                {
                                    value: '标配mic in',
                                },
                            ],
                        },
                        {
                            value: '工作温度和湿度',
                            children: [
                                {
                                    value: '-30℃~60℃,湿度小于95%(无凝结)',
                                },
                            ],
                        },
                        {
                            value: '电源接口',
                            children: [
                                {
                                    value: 'M12 6芯',
                                },
                            ],
                        },
                        {
                            value: '供电方式',
                            children: [
                                {
                                    value: 'DC:9~36V',
                                },
                            ],
                        },
                        {
                            value: '流及功耗:',
                            children: [
                                {
                                    value: 'DC： 24V, 0.31 A, Max：7.4W',
                                },
                            ],
                        },
                        {
                            value: '红外距离',
                            children: [
                                {
                                    value: '3m',
                                },
                            ],
                        },
                        {
                            value: '尺寸(mm)',
                            children: [
                                {
                                    value: '产品尺寸：230*100*50 mm ; 包装尺寸：280*135*135mm',
                                },
                            ],
                        },
                        {
                            value: '重量',
                            children: [
                                {
                                    value: '机身重量：640g ; 带包装重量：960g',
                                },
                            ],
                        },
                    ]
                },





                ]
            },
            {
                title: '电子站牌',
                img: './assets/img/produce/list/6.png',
                image: ['./assets/img/produce/bus6.1.jpg'],
                type: 1,
                summary: '采用高性能工业级无线模块，采用高性能工业级 32 位通信处理器，支持低功耗模式，包括休眠模式、定时上下线模式和定时开关机模式（仅特殊版本支持',
                feature: [{
                    title: '产品功能特点',
                    content: [
                        {
                            title: '工业级应用设计',
                            list: [
                                '采用高性能工业级无线模块',
                                '采用高性能工业级 32 位通信处理器',
                                '支持低功耗模式，包括休眠模式、定时上下线模式和定时开关机模式（仅特殊版本支持）',
                                '采用金属外壳，保护等级 IP30。金属外壳和系统安全隔离，特别适合于工控现场的应用',
                                '宽电源输入（DC 5~35V）',

                            ]
                        },
                        {
                            title: '稳定可靠',
                            list: [
                                'WDT 看门狗设计，保证系统稳定',
                                '采用完备的防掉线机制，保证数据终端永远在线',
                                '以太网接口内置 1.5KV 电磁隔离保护',
                                'RS232/RS485/RS422 接口内置 15KV  ESD保护',
                                'SIM/UIM 卡接口内置 15KV ESD 保护',
                                '电源接口内置反相保护和过压保护',
                                '天线接口防雷保护（可选）',
                            ]
                        },
                        {
                            title: '标准易用',
                            list: [
                                '提供标准 RS232（或RS485/RS422）、以太网和 WIFI 接口，可直接连接串口设备、以太网设备和 WIFI 设备',
                                '智能型数据终端，上电即可进入数据传输状态',
                                '提供功能强大的中心管理软件，方便设备管理（可选）',
                                '使用方便，灵活，多种工作模式选择',
                                '方便的系统配置和维护接口（包括本地和远端WEB和CLI方式）',


                            ]
                        },


                        {
                            title: '产品参数',
                            list: [
                                '支持 3G/HSPA/4G WAN 连接方式。',
                                '支持 VPN sever（PPTP， L2TP， OPENVPN，IPSEC  和 GRE）（注：仅 VPN 版支持）',
                                '支持远程管理，SYSLOG、SNMP、TELNET、SSHD，HTTPS 等功能',
                                '支持本地和远程在线升级，导入导出配置文件。',
                                '支持 NTP，内置 RTC。',
                                '支持国内外多种 DDNS。',
                                '支持 MAC 地址克隆，PPPoE 服务器。',
                                'WIFI 支持 802.11b/g/n,支持 WIFI AP、AP Client，中继器，中继桥接和 WDS 等多种工作模式（可选）',
                                'WIFI 支持 WEP,WPA，WPA2 等多种加密方式，支持 RADIUS 认证， MAC 地址过滤等功能。',
                                '支持多种上下线触发模式，包括短信、电话振铃、串口数据、网络数据触发上下线模式。',
                                '支持 APN/VPDN ',
                                '支持多路 DHCP  server 及 DHCP  client，DHCP 捆绑 MAC 地址， DDNS，防火墙，NAT，DMZ 主机，QoS，流量统计,实时显示数据传输速率等功能',
                                '使用方便，灵活，多种工作模式选择',
                                '方便的系统配置和维护接口（包括本地和远端WEB和CLI方式）',
                                '提供标准 RS232（或RS485/RS422）、以太网和 WIFI 接口，可直接连接串口设备、以太网设备和 WIFI 设备',
                                '支持 TCP/IP、UDP、FTP、HTTP 等多种网络协议',
                                '支持 SPI 防火墙，VPN 穿越，访问控制，URL过滤等功能',
                                '定时开关机，定时上下线功能',



                            ]
                        },
                    ]
                }]

            },
            {
                title: '出租车车载设备主机',
                img: './assets/img/produce/list/7.png',
                image: [
                    './assets/img/produce/ruc1.1.png',
                    './assets/img/produce/ruc1.2.png',
                    './assets/img/produce/ruc1.3.png',
                ],
                type: 1,
                summary:'支持全网通讯，wifi通讯，蓝牙通讯,不对接驾驶行为分析仪时，采用人脸识别方式识别出已注册驾驶员和非法驾驶员,进入运营状态后，与计价器同步显示运营里程和金额',
                feature: [{
                    title: '产品功能特点',
                    content: [
                        {
                            title: '通讯功能',
                            list: [
                                '支持全网通讯，wifi通讯，蓝牙通讯',
                            ]
                        },
                        {
                            title: '人脸识别功能',
                            list: [
                                '不对接驾驶行为分析仪时，采用人脸识别方式识别出已注册驾驶员和非法驾驶员',
                            ]
                        },
                        {
                            title: '计价器通讯功能',
                            list: [
                                '进入运营状态后，与计价器同步显示运营里程和金额',
                            ]
                        },
                        {
                            title: '合乘功能',
                            list: [
                                '根据GPS数据，同时为3组乘客计费并显示',
                            ]
                        },
                        {
                            title: '智能顶灯控制功能',
                            list: [
                                '控制智能顶灯分别显示有客、空车、暂停、合乘',
                            ]
                        },
                        {
                            title: '行车记录仪功能',
                            list: [
                                '连接对内摄像头，对外摄像头，能够同时对车内，车外进行视频录像，并能够回放',
                            ]
                        },
                        {
                            title: 'OBD数据采集器',
                            list: [
                                '关闭GPS，连接车辆OBD接口，车辆通过OBD接口数据计程',
                            ]
                        },



                    ]
                }],

                parameter: [
                    {
                        title: '产品参数',
                        thead: [
                            {
                                value: '名称',
                            },
                            {
                                value: '参数',
                            },

                        ],
                        tbody: [
                            {
                                value: '方案',
                                rowspan: 1,
                                children: [
                                    {
                                        value: 'MTK6737+四核+2G+16G+1080P+4G网络数据+6.86+白蓝TP+GPS外置有源天线',
                                    },
                                ]
                            },

                            {
                                value: '网络频段',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '移动联通2G：GSM(B3和B5),联通3G：B1和B5，FDD：B1、B3、B5，TDD和TD-SCDMA',
                                    },
                                ]
                            },
                            {
                                value: '手机APP',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '车车助手',
                                    },
                                ]
                            },
                            {
                                value: '开机画面LOGO',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '客户LOGO',
                                    },
                                ]
                            },
                            {
                                value: '使用镜头数量',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '一录照车前，一录照车内',
                                    },
                                ]
                            },
                            {
                                value: '前视镜头型号',
                                rowspan: 1,
                                children: [
                                    {
                                        value: 'ps5250++++TRC2117',
                                    },
                                ]
                            },
                            {
                                value: '焦距',
                                rowspan: 1,
                                children: [
                                    {
                                        value: 'f=2.9mm',
                                    },
                                ]
                            },
                            {
                                value: '角度',
                                rowspan: 1,
                                children: [
                                    {
                                        value: 'D：142°；H：121°；V：64°',
                                    },
                                ]
                            },
                            {
                                value: '光圈',
                                rowspan: 1,
                                children: [
                                    {
                                        value: 'F=1.4',
                                    },
                                ]
                            },
                            {
                                value: '车内摄像头',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '走线',
                                    },
                                ]
                            },
                            {
                                value: '变焦（数字/光学)',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '不支持',
                                    },
                                ]
                            },
                            {
                                value: 'LCD显示屏',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '6.86寸 1280x480 IPS',
                                    },
                                ]
                            },
                            {
                                value: '视频尺寸',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '前1920*1080P',
                                    },
                                ]
                            },
                            {
                                value: '视频格式',
                                rowspan: 1,
                                children: [
                                    {
                                        value: 'ts(加锁文件mp4）',
                                    },
                                ]
                            },
                            {
                                value: '拍照画面格式',
                                rowspan: 1,
                                children: [
                                    {
                                        value: 'JPG',
                                    },
                                ]
                            },
                            {
                                value: '麦克风/喇叭',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '内置',
                                    },
                                ]
                            },
                            {
                                value: '充电方式',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '使用宝马线:5V/3A',
                                    },
                                ]
                            },
                            {
                                value: '使用双路录像电流',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '550mA',
                                    },
                                ]
                            },
                            {
                                value: '电池容量',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '无',
                                    },
                                ]
                            },
                            {
                                value: 'GPS',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '支持',
                                    },
                                ]
                            },
                            {
                                value: '导航',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '支持',
                                    },
                                ]
                            },
                            {
                                value: 'Wi-Fi',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '支持',
                                    },
                                ]
                            },
                            {
                                value: '网络数据',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '支持',
                                    },
                                ]
                            },
                            {
                                value: '镜头旋转功能',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '支持',
                                    },
                                ]
                            },
                            {
                                value: '最大支持内存',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '32GB外接TF卡',
                                    },
                                ]
                            },
                            {
                                value: '停车监控',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '支持',
                                    },
                                ]
                            },
                            {
                                value: '远程控制',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '支持',
                                    },
                                ]
                            },
                            {
                                value: '直播功能',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '支持',
                                    },
                                ]
                            },
                            {
                                value: '碰撞加锁',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '支持',
                                    },
                                ]
                            },
                            {
                                value: '循环录像',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '无缝循环录像,不漏秒',
                                    },
                                ]
                            },
                            {
                                value: 'WIFI热点',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '支持',
                                    },
                                ]
                            },
                            {
                                value: 'GPS导航',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '支持',
                                    },
                                ]
                            },
                            {
                                value: 'GPS轨迹',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '支持',
                                    },
                                ]
                            },
                            {
                                value: 'FM发射',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '支持',
                                    },
                                ]
                            },
                            {
                                value: '自动倒车可视',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '支持',
                                    },
                                ]
                            },
                            {
                                value: '电子狗',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '支持固定数据播报',
                                    },
                                ]
                            },
                            {
                                value: 'G-SENSOR',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '高/中/低/关闭',
                                    },
                                ]
                            },
                            {
                                value: '存储温度',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '（-25度至75度）',
                                    },
                                ]
                            },
                            {
                                value: '工作环境温度',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '（-20度至70度）',
                                    },
                                ]
                            },
                            {
                                value: '工作环境湿度',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '15~85%RH',
                                    },
                                ]
                            },
                            {
                                value: '默认光源频率',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '50Hz',
                                    },
                                ]
                            },
                            {
                                value: '按键定义',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '甩线一键报警按键，后排三个评价键，前车评价键在设备上做虚拟按键',
                                    },
                                ]
                            },
                            {
                                value: 'ADAS',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '支持',
                                    },
                                ]
                            },
                            {
                                value: '人脸识别',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '支持',
                                    },
                                ]
                            },
                            {
                                value: 'DMS',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '支持',
                                    },
                                ]
                            },
                            {
                                value: '扩展接口',
                                rowspan: 8,
                                children: [
                                    {
                                        value: '三路RS232，带5V可控电源输出，每录可带500MA负载',
                                    },
                                    {
                                        value: '一路RS485',

                                    },
                                    {
                                        value: '一路CAN接口',

                                    },
                                    {
                                        value: '12V高清摄像电源输入，可带500MA负载',

                                    },
                                    {
                                        value: '一路5PIN的航空头USB，带5V/2A输入',

                                    },
                                    {
                                        value: '六路IO输入',

                                    },
                                    {
                                        value: '一路里程脉冲输入',

                                    },
                                    {
                                        value: '三路可以定义输出',

                                    },
                                ]
                            },


                        ]

                    },


                ]
            },

            // 12
            {
                title: '车载报站屏',
                img: './assets/img/produce/list/12.png',
                type: 1,
                summary: '',
                feature: [{
                    title: '产品概述',
                    content: [
                        {
                            title: '',
                            list: [
                                ''
                            ]
                        },
                    ]
                }],
            },
            // 13
            {
                title: '出租车车顶LED屏',
                img: './assets/img/produce/list/13.png',
                type: 1,
                summary: '',
                feature: [{
                    title: '产品概述',
                    content: [
                        {
                            title: '',
                            list: [
                                ''
                            ]
                        },
                    ]
                }],
            },
            // 14
            {
                title: '4.2.3出租车计价器',
                img: './assets/img/produce/list/14.png',
                type: 1,
                summary: '',
                feature: [{
                    title: '产品概述',
                    content: [
                        {
                            title: '',
                            list: [
                                ''
                            ]
                        },
                    ]
                }],
            },
        ]
    },
    {
        title: '军队产品',
        subtext: 'army',
        children: [
            {
                title: '电子沙盘',
                img: './assets/img/produce/list/10.png',
                image: ['./assets/img/produce/army2.jpg'],
                type: 1,
                summary: '根据武警、部队、公安作战管理需求，以三维地理信息、部队部署、要图标绘等为基础支撑，深度开发强化信息查询、地形分析、三维立体模型标绘、三维动态军标标绘、态势演示、联网标图、动员数据、实时视频、北斗导航、实时语音、防恐防暴动态演示等主要功能，为指挥员处置应急突发事件、正确分析情况、判定态势定下决心，提供有力的支撑。',
                feature: [{
                    title: '产品概述',
                    content: [
                        {
                            title: '',
                            list: [
                                '根据武警、部队、公安作战管理需求，以三维地理信息、部队部署、要图标绘等为基础支撑，深度开发强化信息查询、地形分析、三维立体模型标绘、三维动态军标标绘、态势演示、联网标图、动员数据、实时视频、北斗导航、实时语音、防恐防暴动态演示等主要功能，为指挥员处置应急突发事件、正确分析情况、判定态势定下决心，提供有力的支撑。'
                            ]
                        },
                    ]
                },
                {
                    title: '主要功能',
                    content: [
                        {
                            title: '',
                            list: [
                                '多样手势交互',
                                '电子地图数据本地化',
                                '目标类型多样化',
                                '三维（二维）军标、电子白板 标绘功能',
                                '三维模型',
                                '具备多点触摸测量功能',
                                '动态军标、态势演示',
                                '联网协同标图',
                            ]
                        },
                    ]
                }],
            },
            // 11
            {
                title: '智能手机柜',
                img: './assets/img/produce/list/11.png',
                type: 1,
                summary: '',
                feature: [{
                    title: '产品概述',
                    content: [
                        {
                            title: '',
                            list: [
                                ''
                            ]
                        },
                    ]
                }],
            },
        ]
    },
    {
        title: '教育产品',
        subtext: 'education',
        children: [
          
        ],
    },
    {
        title: '人工智能产品',
        subtext: 'artificial intelligence',
        children: [
            {
                title: '辅助驾驶设配',
                img: './assets/img/produce/list/9.png',
                image: [
                    './assets/img/produce/scheme1.1.png',
                    './assets/img/produce/scheme1.2.png',
                ],
                type: 1,
                summary: '前车碰撞预警（FCW）,车道偏离预警（LDW）,车距监控与预警（HMW）',
                parameter: [
                    {
                        title: '产品功能特点',
                        thead: [
                            {
                                value: '告警类型',
                            },
                            {
                                value: '说明',
                            },
                            {
                                value: '默认使能条件',
                            },
                            {
                                value: '备注',
                            },
                        ],
                        tbody: [
                            {
                                value: '前车碰撞预警（FCW）',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '实时监测前方车距，最早可以在预计发生碰撞前 2.7 秒时警示驾驶员，以预防追尾事故。',
                                        children: [
                                            {
                                                value: '时速≥30km/h',
                                                children: [
                                                    {
                                                        value: '标配',
                                                    },
                                                ],
                                            },
                                        ],
                                    },
                                ]
                            },
                            {
                                value: '车道偏离预警（LDW）',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '监测当前车道标线及车辆轨迹，在即将无意偏离车道时发出警报，提醒驾驶员保持安全车道行驶。',
                                        children: [
                                            {
                                                value: '时速≥55km/h',
                                                children: [
                                                    {
                                                        value: '标配',
                                                    },
                                                ],
                                            },
                                        ],
                                    },
                                ]
                            },
                            {
                                value: '车距监控与预警（HMW）',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '显示前方车辆的车距（以秒为单位）， 在接近预设危险距离时告警',
                                        children: [
                                            {
                                                value: '时速≥30km/h',
                                                children: [
                                                    {
                                                        value: '标配',
                                                    },
                                                ],
                                            },
                                        ],
                                    },
                                ]
                            },
                            {
                                value: '城市前车碰撞预警（UFCW）',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '低速行驶时监测前方车距，当小于安全车距时提醒驾驶员避免擦碰。',
                                        children: [
                                            {
                                                value: '时速＜30km/h',
                                                children: [
                                                    {
                                                        value: '选配',
                                                    },
                                                ],
                                            },
                                        ],
                                    },
                                ]
                            },
                            {
                                value: '行人碰撞预警（PCW）',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '识别前方行人并检测距离，在危险情况下发出警报，提醒驾驶员避免碰撞行人',
                                        children: [
                                            {
                                                value: '时速＜50km/h',
                                                children: [
                                                    {
                                                        value: '选配',
                                                    },
                                                ],
                                            },
                                        ],
                                    },
                                ]
                            },
                            {
                                value: '限速提醒（SLI）',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '识别限速标志牌及其限速值，并在车辆超出规定限速时发出警报，以提醒安全行驶',
                                        children: [
                                            {
                                                value: '/',
                                                children: [
                                                    {
                                                        value: '选配',
                                                    },
                                                ],
                                            },
                                        ],
                                    },
                                ]
                            },
                            {
                                value: '前车启停提醒（STOP&GO）',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '近距停车时（比如排队等红灯）监测前方车辆，当前车由静止启动前行时提醒驾驶员及时启动，以按秩序行驶。',
                                        children: [
                                            {
                                                value: '/',
                                                children: [
                                                    {
                                                        value: '选配',
                                                    },
                                                ],
                                            },
                                        ],
                                    },
                                ]
                            },
                            {
                                value: '急加速/急减速提醒（HAW/HDW）',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '车辆紧急加速/刹车时，提醒驾驶员合理控制速度，避免紧急变速造成安全风险',
                                        children: [
                                            {
                                                value: '/',
                                                children: [
                                                    {
                                                        value: '选配',
                                                    },
                                                ],
                                            },
                                        ],
                                    },
                                ]
                            },
                            {
                                value: '急转弯提醒（HCW）',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '车辆变道或弯道行驶时到达极端状态， 提醒驾驶员即时减速或减少转弯方向以避免事故发生。<br />' +
                                            '1)使能速度可以根据实际使用情况配置；<br />' +
                                            '2)能够在白天、夜晚、隧道、雨天等大多数场景下使用（极端天气除外）<br />' +
                                            '3)支持 ADAS 遮挡失效提示；<br />' +
                                            '3)4)更多其它功能需求，请联系官方技术支持。<br />'
                                        ,
                                        children: [
                                            {
                                                value: '/',
                                                children: [
                                                    {
                                                        value: '选配',
                                                    },
                                                ],
                                            },
                                        ],
                                    },
                                ]
                            },
                        ]

                    },
                    {
                        title: '驾驶员监控功能',
                        thead: [
                            {
                                value: '告警类型',
                            },
                            {
                                value: '说明',
                            },
                            {
                                value: '备注',
                            },
                        ],
                        tbody: [
                            {
                                value: '疲劳驾驶告警',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '识别闭眼持续状态以及打哈欠状态（分成轻度和严重两级）',
                                        children: [
                                            {
                                                value: '标配',
                                            },
                                        ],
                                    },
                                ]
                            },
                            {
                                value: '注意力分散告警',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '识别左右扭头持续状态以及低／抬头状',
                                        children: [
                                            {
                                                value: '标配',
                                            },
                                        ],
                                    },
                                ]
                            },
                            {
                                value: '接打电话告警',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '识别打电话的姿势',
                                        children: [
                                            {
                                                value: '标配',
                                            },
                                        ],
                                    },
                                ]
                            },
                            {
                                value: '抽烟告警',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '识别抽烟的姿势',
                                        children: [
                                            {
                                                value: '标配',
                                            },
                                        ],
                                    },
                                ]
                            },
                            {
                                value: '离岗告警',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '识别驾驶员离开座位',
                                        children: [
                                            {
                                                value: '标配',
                                            },
                                        ],
                                    },
                                ]
                            },
                            {
                                value: '驾驶员变更提示',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '当前检测人脸与上次不同',
                                        children: [
                                            {
                                                value: '标配',
                                            },
                                        ],
                                    },
                                ]
                            },
                            {
                                value: '长时疲劳驾驶告警',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '持续 4 小时/24 小时内驾驶累积 8 小时',
                                        children: [
                                            {
                                                value: '选配',
                                            },
                                        ],
                                    },
                                ]
                            },
                            {
                                value: '未系安全带告警',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '识别安全带的状态',
                                        children: [
                                            {
                                                value: '选配',
                                            },
                                        ],
                                    },
                                ]
                            },
                            {
                                value: '喝水告警',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '识别喝水的姿势',
                                        children: [
                                            {
                                                value: '选配',
                                            },
                                        ],
                                    },
                                ]
                            },
                            {
                                value: '双手脱离方向盘告警',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '检测人的手部是否握住方向盘',
                                        children: [
                                            {
                                                value: '选配（ 需配HOD 摄像头）',
                                            },
                                        ],
                                    },
                                ]
                            },
                            {
                                value: '玩手机告警',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '检测手机物品及人的手臂姿态',
                                        children: [
                                            {
                                                value: '选配（ 需配HOD 摄像头）',
                                            },
                                        ],
                                    },
                                ]
                            },
                            {
                                value: '失效提示',
                                rowspan: 2,
                                children: [
                                    {
                                        value: '设备遮挡失效报警',
                                        children: [
                                            {
                                                value: '标配',
                                            },
                                        ],
                                    },
                                    {
                                        value: '红外眼镜阻断报警',
                                        children: [
                                            {
                                                value: '标配',
                                            },
                                        ],
                                    },
                                ]
                            },
                        ],
                    },
                    {
                        title: '产品参数',
                        thead: [
                            {
                                value: '项目',
                            },
                            {
                                value: '设备参数',
                            },
                        ],
                        tbody: [
                            {
                                value: '额定电压',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '9～36V DC',
                                    },
                                ]
                            },
                            {
                                value: '额定功率',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '10W',
                                    },
                                ]
                            },
                            {
                                value: '工作温度',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '-20～70℃',
                                    },
                                ]
                            },
                            {
                                value: '工作湿度',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '10～90%RH',
                                    },
                                ]
                            },
                            {
                                value: '主机尺寸',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '160mm×105mm×40mm',
                                    },
                                ]
                            },
                            {
                                value: '材质',
                                rowspan: 1,
                                children: [
                                    {
                                        value: 'PC 塑胶、铝合金、电解板',
                                    },
                                ]
                            },
                            {
                                value: '应用处理器',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '八核 ARM@Cortex-A53 处理器',
                                    },
                                ]
                            },
                            {
                                value: '闪存',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '16G Byte',
                                    },
                                ]
                            },
                            {
                                value: '内存',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '2G Byte',
                                    },
                                ]
                            },
                            {
                                value: '网络',
                                rowspan: 1,
                                children: [
                                    {
                                        value: 'FDD-LTE: B1/B3/B7/B20 <br/>' +
                                            'TDD-LTE: B38/B39/B40/B41 <br/>' +
                                            'TDSCDMA：B34/B39 <br/>' +
                                            'WCDMA: B1/8 <br/>' +
                                            'GSM:850/900/1800/1900 <br/>',
                                    },
                                ]
                            },
                            {
                                value: '定位',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '支持 GPS/北斗',
                                    },
                                ]
                            },
                            {
                                value: '喇叭',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '内置，也预留外接',
                                    },
                                ]
                            },
                            {
                                value: 'TF 卡（客户自配）',
                                rowspan: 1,
                                children: [
                                    {
                                        value: '支持 256G Class 10',
                                    },
                                ]
                            },
                        ],
                    },
                ]
            },
            // 5
            {
                title: '人脸识别系统',
                img: './assets/img/produce/list/5.png',
                type: 1,
                summary: '',
                feature: [{
                    title: '产品概述',
                    content: [
                        {
                            title: '',
                            list: [
                                ''
                            ]
                        },
                    ]
                }],
            },
        ],
    },
];